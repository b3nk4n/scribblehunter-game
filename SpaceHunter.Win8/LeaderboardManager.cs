...
using ScribbleHunter.Windows.LiveTile;
...
namespace ScribbleHunter
{
    class LeaderboardManager
    {
        ...
        public const string TEXT_RESUBMIT_EXIST = "Already submitted!";
        ...
        private const string PC_SIGN = "PC";
        ...
        public async void Submit(string method, string name, long score, int level)
        {
            bool error = false;
            string content = string.Empty;
            try
            {
                HttpResponseMessage data = await submitData(method, name, score, level);
            ...
            handleDownloadCompleted(content, error);
        }
        ...
        /// <param name="score">The players score.</param>
        /// <param name="level">The players level.</param>
        private async Task<HttpResponseMessage> submitData(string method, string name, long score, int level)
        {
            string phoneid = PHONE_ID;

            string version = PC_SIGN;
            ...
            return await client.GetAsync(new Uri(string.Format(SUBMIT_FORMAT,
                                                                         method,
                                                                         phoneid,
                                                                         name,
                                                                         score,
        ...
        public async void Receive()
        {
            ...
            try
            {
                HttpResponseMessage data = await receiveData();
                content = await data.Content.ReadAsStringAsync();
            }
            catch (Exception)
            {
        ...
        private async Task<HttpResponseMessage> receiveData()
        {
            statusText = TEXT_START_REFRESH;
            return await client.GetAsync(new Uri(string.Format(RECEIVE_FORMAT,
                                                               PHONE_ID)));
        }
        ...
        /// </summary>
        void handleDownloadCompleted(string content, bool error)
        {
            if (error)
            {
            ...
            if (res.Length > 10)
            {
                ...
                LiveTileHelper.ShowHighscoreLiveTile(topRankMe, topScoreMe);
        ...
        private async void saveXml()
        {
            if (!string.IsNullOrEmpty(xml))
            {
                ...
                var file = await folder.CreateFileAsync(LEADERBOARDS_FILE, CreationCollisionOption.OpenIfExists);
        ...
        private async void loadXml()
        {
            var folder = ApplicationData.Current.LocalFolder;
            var file = await folder.CreateFileAsync(LEADERBOARDS_FILE, CreationCollisionOption.OpenIfExists);
            xml = await FileIO.ReadTextAsync(file);
        }
