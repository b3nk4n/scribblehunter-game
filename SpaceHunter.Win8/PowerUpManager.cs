...
namespace ScribbleHunter
{
    class PowerUpManager : ILevel
    {
        ...
        private Vector2 getRandomPosition()
        {
            ...
            do
            {
                int x = rand.Next(30, 770);
                int y = rand.Next(75, 450);
                pos = new Vector2(x, y);
        ...
        {
            // Powerups
            int powerUpsCount = Int32.Parse(data.Dequeue());
            ...
            for (int i = 0; i < powerUpsCount; ++i)
            {
                ...
                type = (PowerUp.PowerUpType)Enum.Parse(type.GetType(), data.Dequeue(), false);
                PowerUp p = new PowerUp(texture,
                                    Vector2.Zero,
            ...
            this.spawnTimerMin = Single.Parse(data.Dequeue());
            this.spawnTimerMax = Single.Parse(data.Dequeue());
        ...
        {
            // Powerups
            data.Enqueue(powerUps.Count.ToString());

            for (int i = 0; i < powerUps.Count; ++i)
            {
                data.Enqueue(powerUps[i].Type.ToString());
                powerUps[i].Deactivated(data);
            }

            data.Enqueue(spawnTimer.ToString());
            data.Enqueue(spawnTimerMin.ToString());
            data.Enqueue(spawnTimerMax.ToString());
