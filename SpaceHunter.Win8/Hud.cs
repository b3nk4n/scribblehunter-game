...
namespace ScribbleHunter
{
    class Hud
    {
        ...
        private readonly Vector2 ScoreLocation = new Vector2(32, 3);
        private readonly Vector2 ComboLocation = new Vector2(5, 25);
        private readonly Vector2 UpgradesLocation = new Vector2(645, 10);

        private readonly Rectangle UpgradeSymbolDestination = new Rectangle(618, 4, 24, 24);
        ...
        private Vector2 playerReloadLocation = new Vector2(0, 460);
        ...
        private const int ReloadBarSourceWidth = 480;
        private const int ReloadBarDestinationWidth = 800;
        ...
        {
            float sourceFactor = ReloadBarSourceWidth / reloadValueMax;
            float destinationFactor = ReloadBarDestinationWidth / reloadValueMax;
            ...
            spriteBatch.DrawBordered(
                texture,
                    new Rectangle(
                        (int)playerReloadLocation.X,
                        (int)playerReloadLocation.Y,
                        (int)(destinationFactor * value),
                        ReloadBarHeight),
                    new Rectangle(
                        (int)BigBarOverlayStart.X,
                        (int)BigBarOverlayStart.Y,
                        (int)(sourceFactor * value),
                        ReloadBarHeight),
                    Color.Purple * 0.75f,
                    Color.White * 0.75f);
        }
        ...
        private void drawCanFire(SpriteBatch spriteBatch)
        {
            if (displayCanFireTimer > 0)
            {
                ...
                spriteBatch.Draw(
                    texture,
                    new Rectangle(0, 0, 800, 4),
                    new Rectangle(0, 550, 4, 4),
                    Color.Purple * colorFactor);
                spriteBatch.Draw(
                    texture,
                    new Rectangle(0, 476, 800, 4),
                    new Rectangle(0, 550, 4, 4),
                    Color.Purple * colorFactor);
                spriteBatch.Draw(
                    texture,
                    new Rectangle(0, 4, 4, 472),
                    new Rectangle(0, 550, 4, 4),
                    Color.Purple * colorFactor);
                spriteBatch.Draw(
                    texture,
                    new Rectangle(796, 4, 4, 472),
                    new Rectangle(0, 550, 4, 4),
                    Color.Purple * colorFactor);
            }
        }
