...
namespace ScribbleHunter
{
    class LevelManager
    {
        ...
        {
            this.levelTimer = Single.Parse(data.Dequeue());
            currentLevel = Int32.Parse(data.Dequeue());
            this.lastLevel = Int32.Parse(data.Dequeue());
            this.hasChanged = Boolean.Parse(data.Dequeue());
            this.levelState = (LevelStates)Enum.Parse(levelState.GetType(), data.Dequeue(), false); 
        }

        public void Deactivated(Queue<string> data)
        {
            data.Enqueue(levelTimer.ToString());
            data.Enqueue(currentLevel.ToString());
            data.Enqueue(lastLevel.ToString());
            data.Enqueue(hasChanged.ToString());
            data.Enqueue(levelState.ToString());
        }
