...
namespace ScribbleHunter
{
    class ShotManager
    {
        ...
        {
            // Shots
            int shotsCount = Int32.Parse(data.Dequeue());

            Shots.Clear();

            for (int i = 0; i < shotsCount; ++i)
            {
                Vector2 location = new Vector2(Single.Parse(data.Dequeue()),
                                               Single.Parse(data.Dequeue()));
                Vector2 direction = new Vector2(Single.Parse(data.Dequeue()),
                                                Single.Parse(data.Dequeue()));
                ...
                FireShot(location,
                         direction,
                         true,
                         new Color(Int32.Parse(data.Dequeue()),
            ...
            // Rockets
            int rocketsCount = Int32.Parse(data.Dequeue());
            ...
            this.rocketSmokeTimer = Single.Parse(data.Dequeue());

            // Sonic
            int sonicCount = Int32.Parse(data.Dequeue());
            ...
            // Mines
            int minesCount = Int32.Parse(data.Dequeue());
        ...
        {
            // Shots
            data.Enqueue(Shots.Count.ToString());

            for (int i = 0; i < Shots.Count; ++i)
            {
                data.Enqueue(Shots[i].Location.X.ToString());
                data.Enqueue(Shots[i].Location.Y.ToString());
                data.Enqueue(Shots[i].Velocity.X.ToString());
                data.Enqueue(Shots[i].Velocity.Y.ToString());
                data.Enqueue(((int)Shots[i].TintColor.R).ToString());
                data.Enqueue(((int)Shots[i].TintColor.G).ToString());
                data.Enqueue(((int)Shots[i].TintColor.B).ToString());
                data.Enqueue(((int)Shots[i].TintColor.A).ToString());
            }

            // Rockets
            data.Enqueue(Rockets.Count.ToString());

            for (int i = 0; i < Rockets.Count; ++i)
            {
                data.Enqueue(Rockets[i].Location.X.ToString());
                data.Enqueue(Rockets[i].Location.Y.ToString());
                data.Enqueue(Rockets[i].Velocity.X.ToString());
                data.Enqueue(Rockets[i].Velocity.Y.ToString());
                data.Enqueue(((int)Rockets[i].TintColor.R).ToString());
                data.Enqueue(((int)Rockets[i].TintColor.G).ToString());
                data.Enqueue(((int)Rockets[i].TintColor.B).ToString());
                data.Enqueue(((int)Rockets[i].TintColor.A).ToString());
            }

            data.Enqueue(rocketSmokeTimer.ToString());

            // Sonic
            data.Enqueue(Sonic.Count.ToString());

            for (int i = 0; i < Sonic.Count; ++i)
            {
                data.Enqueue(Sonic[i].Location.X.ToString());
                data.Enqueue(Sonic[i].Location.Y.ToString());
                data.Enqueue(Sonic[i].Velocity.X.ToString());
                data.Enqueue(Sonic[i].Velocity.Y.ToString());
                data.Enqueue(((int)Sonic[i].TintColor.R).ToString());
                data.Enqueue(((int)Sonic[i].TintColor.G).ToString());
                data.Enqueue(((int)Sonic[i].TintColor.B).ToString());
                data.Enqueue(((int)Sonic[i].TintColor.A).ToString());
            }

            // Mines
            data.Enqueue(Mines.Count.ToString());

            for (int i = 0; i < Mines.Count; ++i)
            {
                data.Enqueue(Mines[i].Location.X.ToString());
                data.Enqueue(Mines[i].Location.Y.ToString());
                data.Enqueue(((int)Mines[i].TintColor.R).ToString());
                data.Enqueue(((int)Mines[i].TintColor.G).ToString());
                data.Enqueue(((int)Mines[i].TintColor.B).ToString());
                data.Enqueue(((int)Mines[i].TintColor.A).ToString());
            }
        }
